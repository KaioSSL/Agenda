...
class ConnectionFactory {
	...
	var $dbase = "Agenda"; # Nome do seu Banco de Dados
    ...
    function comboContatos($_query){
            $this->conecta();
            $_consulta = mysql_query($_query);
            while($_registros = mysql_fetch_array($_consulta)){
                echo "<option value=" . $_registros['id_contato'].">".$_registros['nome']."</option>";
                }
            }
    function comboBoxMod($_fabricante){
        $this->conecta();
        $_query = "SELECT * FROM veiculo WHERE fabricante = '$_fabricante'";
        $_consulta = mysql_query($_query);
        while($_registros = mysql_fetch_array($_consulta)){
            echo "<option value=" . $_registros[2].">".$_registros[2]."</option>";

                }
        
    }
    function comboBoxVersao($_modelo){
        $this->conecta();
        $_query = "SELECT * FROM veiculo WHERE veiculo = '$_modelo'";
        $_consulta = mysql_query($_query);
        while($_registros = mysql_fetch_array($_consulta)){
            echo "<option value=" . $_registros[2].">".$_registros[1]."</option>";
